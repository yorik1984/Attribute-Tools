...
module AddAttributes
  ...
  def self.set_dynamic_attributes(entity, input)
    ...
    if input.has_key?('value'.to_sym) && (!input[:units].to_s.empty? && !include_element?(without_access, input[:label].to_s))
      if include_element?(wide_label, input[:label].to_s)
    ...
    unless result_value.nil?
      ...
      elsif include_element?(wide_label, input[:label].to_s)
    ...
    entity.set_attribute dict, "_#{label_input}_options", input[:options].to_s unless input[:options].to_s.empty?
  ...
  def self.help_information
    ...
    help_file = File.join('file://', plugins, help_file_folder, 'help.html')
    if help_file
