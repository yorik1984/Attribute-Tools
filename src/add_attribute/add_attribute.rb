...
class AddAttributeInputbox
  ...
  def inputbox(choice, selection)
    ...
    @list       += ['Ignore|Replace|Equal replace', @recursive_level_list, 'Off|Short in messagebox|Full in console']
...
def self.recursive_set_dynamic_attributes(selection, input, duplicate_status, recursive_level, current_nested_level = 2, listing_components = [])
  ...
  selection.each do |entity|
    ...
    listing_components = recursive_set_dynamic_attributes(definition.entities,
      input, duplicate_status, recursive_level, current_nested_level + 1, listing_components)
  end
  listing_components
end
...
def self.set_dynamic_attributes(entity, input)
  ...
  without_access = %w[Name Summary Description ItemCode Material ScaleTool Hidden onClick Copies DialogWidth DialogHeight]
