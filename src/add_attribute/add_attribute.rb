...
# Copyright 2020, Yurij Kulchevich aka yorik1984
# All Rights Reserved
...
# 2.0  5-November-2020  - New features release
#  - mulpiply attributes adding
#  - load pre-settings
# ------------------------------------------------------------------------------
...
module AddAttributes
  class AddAttributeInputbox
    ...
    def inputbox(choice, selection)
      ...
      when 'X', 'Y', 'Z', 'LenX', 'LenY', 'LenZ'
        @inputbox_window_name = if %w[X Y Z].include?(choice)
      ...
      when 'Copies', 'ImageURL'
        @inputbox_window_name = if choice == 'Copies'
      ...
      @prompts += [@prompts_all[:duplicate], @prompts_all[:recursive], @prompts_all[:report]]
      @defaults += %w[Ignore 2 Off]
      @list += ['Ignore|Replace|Equal replace', @recursive_level_list,
  ...
  def self.recursive_set_dynamic_attributes(selection, input, duplicate_status, recursive_level, current_nested_level = 2, listing_components = [])
    ...
    selection.each do |entity|
      ...
      if current_nested_level <= recursive_level && entity.is_a?(Sketchup::ComponentInstance)
        separator = '  '
      ...
      listing_components = recursive_set_dynamic_attributes(definition.entities, input, duplicate_status,
                                                            recursive_level, current_nested_level + 1, listing_components)
    end
    listing_components
  end
  ...
  def self.set_dynamic_attributes(entity, input)
    ...
    without_access = %w[Name Summary Description ItemCode Material ScaleTool Hidden onClick
                        Copies DialogWidth DialogHeight]
    ...
    if input.has_key?('access'.to_sym) && !include_element?(without_access, input[:label].to_s)
      if include_element?(wide_label, input[:label].to_s)
    ...
    if input.has_key?('value'.to_sym) && (input[:units].to_s.length != 0 && !include_element?(without_access,
                                                                                              input[:label].to_s))
      if include_element?(wide_label, input[:label].to_s)
    ...
    case result_value[0].to_s
    when '='
      if input[:formulaunits].to_s.length != 0
        result_value = case input[:formulaunits].to_s
                       when 'Inches'
                         input[:value].to_f * 1.to_inch
                       when 'Centimeters'
                         input[:value].to_f * (1.to_inch / 1.to_cm)
                       else
                         input[:value]
                       end
        if include_element?(wide_formlabel, input[:label].to_s)
    ...
    # entity.bounds.
    zero_point = entity.transformation.origin
    bbox_w = entity.bounds.width.to_inch.to_f
    bbox_h = entity.bounds.height.to_inch.to_f
    bbox_d = entity.bounds.depth.to_inch.to_f
    $dc_observers.get_latest_class.redraw_with_undo(entity) if input_is_standart[0]
    case input[:label].to_s
    when 'LenX'
      entity.set_attribute dict, label_input, bbox_w
      entity.definition.set_attribute dict, label_input, bbox_w
    when 'LenY'
      entity.set_attribute dict, label_input, bbox_h
      entity.definition.set_attribute dict, label_input, bbox_h
    when 'LenZ'
      entity.set_attribute dict, label_input, bbox_d
      entity.definition.set_attribute dict, label_input, bbox_ds
    when 'X'
      entity.set_attribute dict, label_input, zero_point.x
      entity.definition.set_attribute dict, label_input, zero_point.x
    when 'Y'
      entity.set_attribute dict, label_input, zero_point.y
      entity.definition.set_attribute dict, label_input, zero_point.y
    when 'Z'
      entity.set_attribute dict, label_input, zero_point.z
      entity.definition.set_attribute dict, label_input, zero_point.z
      #      when 'RotX'
    ...
    entity.set_attribute dict, "_#{label_input}_options", input[:options].to_s if input[:options].to_s.length != 0
  ...
  def self.inputbox_attributes
    ...
    if select_components_messagebox?(selection)
      ...
      total_count = recursive_set_dynamic_attributes(selection, input, duplicate_status, recursive_level)
      case report
  ...
  def self.help_information
    ...
    help_file = File.join('file://' + plugins, __FILE__, '/help/help.html')
    if help_file
  ...
  unless file_loaded?(__FILE__)
    ...
    inputbox_attributes_cmd = UI::Command.new('Adding new attribute from inputbox') do
      AddAttributes.inputbox_attributes
    end
